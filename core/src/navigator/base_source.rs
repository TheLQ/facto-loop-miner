...
impl BaseSourceEighth {
    ...
    fn get_for_index(&self, index: i32) -> BaseSourceEntry {
        ...
        let applied_infra_offset_axis =
            self.sign * (TOTAL_INTRA_RAILS - (index % TOTAL_INTRA_RAILS)) * INTRA_OFFSET;
        let pos_orig = self.origin.point().move_direction_sideways_int(
            self.origin.direction(),
            self.sign * SECTION_POINTS_I32 * (index / TOTAL_INTRA_RAILS),
        );
        pos_orig.assert_step_rail();

        // intra offset
        let pos = pos_orig
            .move_direction_sideways_int(self.origin.direction(), applied_infra_offset_axis)
            .move_direction_int(self.origin.direction(), applied_infra_offset_axis);
        let applied_intra_offset = pos - pos_orig;
