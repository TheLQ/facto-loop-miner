...
impl Step for Step10 {
    ...
    fn transformer(&self, params: StepParams) -> XMachineResult<()> {
        ...
        surface.patches_mut_fn(|mut s| {
            if REMOVE_RADIUS {
                s.remove_patches_within_radius(tunables.resource_clear_chunks.as_tiles_u32());
            } else {
                s.remove_patches_in_column(tunables.resource_clear_chunks.as_tiles_u32());
            }
            s.pixels_mut_fn(|s| {
                draw_mega_box(s, tunables);
            });
        });
...
fn draw_mega_box(mut surface: VSurfacePixelMut, tunables: &BaseTunables) {
