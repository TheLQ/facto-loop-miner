...
mod test {
    ...
    fn radius_checks() {
        ...
        assert_eq!(surface.pixels().get_pixel(extreme_top_left), Pixel::Empty);
        assert_eq!(
            surface.pixels().get_pixel(extreme_bottom_right),
            Pixel::Empty
        );
        ...
        assert!(!surface.pixels().is_point_out_of_bounds(&extreme_top_left));
        assert!(
            !surface
                .pixels()
                .is_point_out_of_bounds(&extreme_bottom_right)
        );
    }
}
