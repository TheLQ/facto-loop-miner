...
`loop-miner` generates a loop network from exported map data.
Output is a standalone mod to build network via `/loopminer` command.
...
A point2point rail network of un-intersecting loops avoids bottleneck problems in other designs at scale.
